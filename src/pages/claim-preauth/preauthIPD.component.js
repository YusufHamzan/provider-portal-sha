...
export default function ClaimsPreAuthIPDComponent(props) {
  ...
  const handleInputChangeProvider = (e, index) => {
    ...
    const isValAlreadyPresent = providerDetailsList.some(item => item.providerId === value);
  ...
  const handleEstimateChangeProvider = (i, idx, e) => {
  ...
  const handleRemoveProviderWithBenefit = (i, idx) => {
    if (i >= 0 && i < providerDetailsList.length && idx >= 0 && idx < providerDetailsList[i].benefit.length) {
      const updatedA = [...providerDetailsList];
  ...
  const handleBenefitChange = (index, val) => {
    const isOptionPresent = selectedBenefit.some(item => item?.value === val?.value);
    ...
    const isValAlreadyPresent = benefitsWithCost.some(item => item.benefitId === val?.value);
  ...
  const handleBenefitChangeInProvider = (i, idx, val) => {
    if (i >= 0 && i < providerDetailsList.length && idx >= 0 && idx < providerDetailsList[i].benefit.length) {
      const isValAlreadyPresent = providerDetailsList[i].benefit.some(benefitItem => benefitItem.benefitId === val?.value);
            ...
            <Grid container spacing={3} style={{ marginBottom: '20px' }}>
              ...
              <Grid item xs={12} sm={3} style={{display:"flex", alignItems:"flex-end", marginBottom:"8px"}}>
                <Autocomplete
                  ...
                  renderInput={params => <TextField {...params} label="Other Diagnoses" placeholder="Select Diagnosis" />}
                />
              </Grid>
            </Grid>
