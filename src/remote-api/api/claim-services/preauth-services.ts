...
export class PreAuthService {
  ...
  directApprove(payload: any): Observable<Map<string, any>> {
    return http
      .post<Map<string, any>>(
        `${this.COMMAND_CONTEXT}/preauths/sha-approved-preauth`,
        payload
      )
      .pipe(map((response) => response.data));
  }
  ...
  getPreauthTeriffAmount(
    ...
    return http
      .get<Page<any>>(
        `${this.QUERY_CONTEXT}/preauths/sha-subbenefit-interventionCode/${BenefitId}/${interventionId}/${providerLevel}`
      )
      .pipe(map((response) => response.data));
  }
