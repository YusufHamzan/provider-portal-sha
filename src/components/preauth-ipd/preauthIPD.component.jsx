...
export default function ClaimsPreAuthIPDComponent(props) {
  ...
  const handleInitiate = () => {
    ...
    const payload = {
      ...
      "notification_callback_url": "https://api.eoxegen.com/member-command-service/v1/public/sha-member/biometric/callback",
          ...
          <Grid container spacing={3} style={{ marginBottom: "20px" }}>
            <Grid item xs={12} container spacing={2}>
              <Grid item xs={12} sm={4}>
                <Box sx={{ position: 'relative', p: 2, border: '1px solid #ccc', borderRadius: 2 }}>
                  <Typography variant="subtitle1">Member Identification</Typography>
                  {memberIdentified ? (
              ...
              <Grid item xs={12} sm={4}>
                <Box sx={{ position: 'relative', p: 2, border: '1px solid #ccc', borderRadius: 2 }}>
                  <Grid container alignItems='center' >
                    <Typography variant="subtitle1" style={{ marginRight: '12px' }}>Member Biometric</Typography>
                    {biometricInitiated && biometricResponseId ? (
                      <Button label="Check status" severity="help" text onClick={handleCheckStatus} />
                    ) : (
                      <Button label="Initiate" severity="help" text onClick={handleInitiate} />
                    )}
                    {biometricVerified ? (
                      <CheckCircle sx={{ position: 'absolute', top: 8, right: 8, color: 'green' }} />
                    ) : (
              ...
              <Grid item xs={12} sm={4}>
                <Box sx={{ position: 'relative', p: 2, border: '1px solid #ccc', borderRadius: 2 }}>
                  <Typography variant="subtitle1">Member Contribution</Typography>
                  {contributionPaid ? (
                    <CheckCircle sx={{ position: 'absolute', top: 8, right: 8, color: 'green' }} />
                  ) : (
                    <ErrorIcon sx={{ position: 'absolute', top: 8, right: 8, color: 'red' }} />
